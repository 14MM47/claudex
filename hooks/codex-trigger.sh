...
# claudex subagent (Claudex, the Codex reconciler). Pure text injection — no
# network, no billing.
#
...
if printf '%s' "$prompt" | grep -iqE \
   -e '(check|verify|reconcile|confirm|cross[ -]?check|sanity[ -]?check|run (this|it|that)|second opinion|double[ -]?check)[^.]{0,40}(with|by) (codex|claudex)' \
   -e '(ask|consult|query|loop in|run by) +(codex|claudex)' \
   -e "(codex|claudex)('?s)?[^.]{0,30}(opinion|take|sign[ -]?off|to review|(should )?review (this|the|it|that|my))" \
   -e '(get|grab|want|need)[^.]{0,20}(second )?(opinion|take|view)[^.]{0,30}(codex|claudex)' \
   -e '(run|use|launch|invoke) +claudex' ; then
  trigger=1
fi
...
If that reading is correct, handle it via the `claudex` subagent (Claudex, the Codex
reconciler — or the `/check-with-codex` command) rather than a one-off codex call:
launch the Agent with subagent_type "claudex", passing the issue, your current position with
